...
# from backend.services.secure_api_service_corrigido import SecureAPIService # Temporariamente comentado
from backend.middleware.auth_middleware import AuthMiddleware
from backend.auth.login import ensure_admin_credentials
from backend.models.invite_code import initialize_invite_codes # Importar a função de inicialização de convites
...
def create_app(config_name='default'):
    ...
    # --- Configuração do Banco de Dados (APENAS PostgreSQL PRODUÇÃO) ---
    database_url = os.environ.get('DATABASE_URL')
    
    if not database_url:
        raise ValueError("❌ DATABASE_URL não configurada! Este projeto requer PostgreSQL em produção.")
    
    # Corrigir URL se necessário (Render pode usar 'postgres://' mas SQLAlchemy espera 'postgresql://')
    if database_url.startswith('postgres://'):
        database_url = database_url.replace('postgres://', 'postgresql://', 1)
    
    app.config['SQLALCHEMY_DATABASE_URI'] = database_url
    app.logger.info("✅ Conectando ao banco de dados PostgreSQL em produção.")
    ...
    # secure_api_service = SecureAPIService(app)  # Temporariamente comentado
    ...
    # Registrar blueprint de credenciais
