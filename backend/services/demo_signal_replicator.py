...
class DemoSignalReplicator:
    ...
    def replicate_signal(self, signal: ActiveSignal):
        ...
        try:
            ...
            if not demo_user:
                self.logger.error("Usuário demo não encontrado")
                return False
            ...
            if result['success']:
                self.logger.info(f"Sinal {signal.id} ({signal.symbol}) replicado com sucesso na conta demo")
                return True
            else:
                self.logger.error(f"Erro ao replicar sinal {signal.id}: {result.get('message', 'Erro desconhecido')}")
                return False
                
        except Exception as e:
            self.logger.error(f"Erro ao replicar sinal {signal.id}: {str(e)}")
            return False
    ...
    def replicate_all_active_signals(self):
        ...
        try:
            ...
            if not active_signals:
                self.logger.info("Nenhum sinal ativo encontrado para replicar")
                return True
            ...
            self.logger.info(f"Replicação concluída: {success_count}/{total_signals} sinais replicados com sucesso")
            return success_count == total_signals
            
        except Exception as e:
            self.logger.error(f"Erro ao replicar sinais ativos: {str(e)}")
            return False
    ...
    def monitor_new_signals(self):
        ...
        try:
            ...
            if new_signals:
                self.logger.info(f"Encontrados {len(new_signals)} novos sinais para replicar")
            ...
            self.logger.error(f"Erro ao monitorar novos sinais: {str(e)}")
            return False
    ...
    def get_demo_performance(self):
        ...
        try:
            ...
            self.logger.error(f"Erro ao obter performance demo: {str(e)}")
            return None
...
def replicate_signal_to_demo(signal_id: int):
    ...
    try:
        ...
        logging.error(f"Erro ao replicar sinal {signal_id}: {str(e)}")
        return False
