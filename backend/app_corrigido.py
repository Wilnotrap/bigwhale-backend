...
def create_app(config_name='default'):
    ...
    app.config.from_mapping(
        ...
        SESSION_COOKIE_SECURE=is_production,  # True apenas para produção com HTTPS
    ...
    # Configuração do banco de dados - SEMPRE SQLite no Render
    if os.getenv('RENDER'):
        # No Render, usar SQLite em /tmp
        db_path = '/tmp/site.db'
        app.config['SQLALCHEMY_DATABASE_URI'] = f'sqlite:///{db_path}'
    else:
        # Desenvolvimento local
        app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///backend/instance/site.db'
    
    # Remover completamente a seção de PostgreSQL
    app.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = False
    app.logger.info(f"Usando SQLite: {db_path}")
    ...
    # Middleware CORS adicional para garantir
    @app.after_request
    ...
    try:
        ...
        # Usar sessões em memória como fallback
        app.config['SESSION_TYPE'] = 'null'
        app.config['SESSION_FILE_THRESHOLD'] = 500
        app.config['SESSION_FILE_MODE'] = 0o600  # Usar notação octal correta
        app.config['PERMANENT_SESSION_LIFETIME'] = timedelta(days=30)
    ...
    # Importar monitoramento de alvos (com tratamento de erro)
    try:
        import sys
        import os
        sys.path.append(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
        from monitoramentoalvos.api_routes import monitoramento_bp
        MONITORAMENTO_DISPONIVEL = True
    except ImportError as e:
        print(f"⚠️ Módulo de monitoramento não disponível: {e}")
        MONITORAMENTO_DISPONIVEL = False
    ...
    # Registrar monitoramento se disponível
    if MONITORAMENTO_DISPONIVEL:
        app.register_blueprint(monitoramento_bp)
        print("✅ Módulo de monitoramento de alvos integrado")
    ...
    try:
        from services.auto_monitor_service import init_auto_monitor
        from services.nautilus_auto_sync import init_nautilus_sync
        
        # Inicializar sincronização automática com Nautilus
        nautilus_sync_service = init_nautilus_sync(app)
        app.logger.info("🔄 Serviço de sincronização automática com Nautilus iniciado")
        
        # Inicializar monitoramento automático de alvos
        auto_monitor_service = init_auto_monitor(app)
        app.logger.info("🤖 Serviço de monitoramento automático de alvos iniciado")
        
    except Exception as e:
        app.logger.error(f"Erro ao inicializar serviços automáticos: {e}")
        # Continuar sem os serviços automáticos se houver erro
    ...
    def check_session():
        ...
        try:
            ...
            app.logger.error(f"Erro ao verificar sessão: {str(e)}")
            return jsonify({
                'authenticated': False,
                'error': 'Erro interno do servidor'
            }), 500

    # --- Rota do Dashboard - Operações Nautilus ---
    @app.route('/api/dashboard/nautilus-operacional/active-operations', methods=['GET', 'OPTIONS'])
    def nautilus_operations():
        """Endpoint para buscar operações ativas do Nautilus"""
        if request.method == 'OPTIONS':
            return '', 200
            
        try:
            from services.nautilus_auto_sync import get_nautilus_operations
            
            operations = get_nautilus_operations()
            ...
            app.logger.error(f"Erro ao buscar operações Nautilus: {str(e)}")
            return jsonify({
                'success': False,
    ...
    with app.app_context():
        try:
            ...
            # Criar banco PostgreSQL diretamente (apenas em produção)
            if is_production:
                try:
                    app.logger.info("🔄 Criando banco PostgreSQL...")
                    
                    from create_postgres_database import create_postgres_database
                    create_postgres_database()
                    
                    app.logger.info("✅ Banco PostgreSQL criado com sucesso!")
                    
                except Exception as db_error:
                    app.logger.error(f"Erro ao criar banco PostgreSQL: {db_error}")
                    # Continuar mesmo com erro
            else:
                app.logger.info("🔄 Pulando criação do PostgreSQL (desenvolvimento local)")
...
app = create_app()
application = app  # Alias para o Render
