...
def create_app(config_name='default'):
    ...
    # Prioriza a DATABASE_URL do Render (PostgreSQL), senão usa SQLite local.
    database_url = os.environ.get('DATABASE_URL')
    if database_url:
        # O Heroku e o Render usam 'postgres://', mas o SQLAlchemy espera 'postgresql://'
        if database_url.startswith("postgres://"):
            database_url = database_url.replace("postgres://", "postgresql://", 1)
        app.config['SQLALCHEMY_DATABASE_URI'] = database_url
        app.logger.info("Conectando ao banco de dados PostgreSQL.")
    else:
        ...
        app.logger.info(f"DATABASE_URL não encontrada. Usando SQLite em: {db_path}")
