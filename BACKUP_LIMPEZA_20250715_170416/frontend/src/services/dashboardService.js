...
class DashboardService {
  ...
  async reconnectApi() {
    try {
      // Chama o endpoint correto para buscar as credenciais criptografadas
      const response = await axios.get(`${API_BASE_URL}/api/credentials/status`, { withCredentials: true });
      const data = response.data;
      if (data.api_configured) {
        // Retorna sucesso e as credenciais para alimentar o perfil/dashboard
        return { success: true, ...data };
      } else {
        return { success: false, message: 'Credenciais de API não configuradas. Configure no perfil.' };
      }
    } catch (error) {
      ...
      throw new Error(error.response?.data?.message || 'Erro ao conectar API.');
    }
  }

  // Novo método para verificar status das credenciais
  // Método checkCredentialsStatus corrigido
  async checkCredentialsStatus() {
    try {
      ...
      throw error.response?.data || { message: 'Erro ao verificar credenciais' };
    }
  }
}
