...
const Login = () => {
  ...
  // Código removido: todos os botões agora redirecionam para /payment-success
  ...
  const handlePaymentClick = (link) => {
    // CORREÇÃO: Redirecionar para payment-success igual ao teste webhook
    const returnUrl = `${window.location.origin}/payment-success`;
    const cancelUrl = `${window.location.origin}/login`;
    const enhancedLink = `${link}?success_url=${encodeURIComponent(returnUrl)}&cancel_url=${encodeURIComponent(cancelUrl)}`;
    
    window.open(enhancedLink, '_blank', 'noopener,noreferrer');
    
    // Mostrar toast informativo
    toast.success('🚀 Redirecionando para pagamento. Após confirmar, você será direcionado para o cadastro sem código de convite.');
  };
