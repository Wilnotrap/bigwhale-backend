...
# -*- coding: utf-8 -*-
"""
Correção Definitiva para Persistência da API
Esta correção substitui o APIPersistence por uma solução baseada apenas em SQLAlchemy,
eliminando o problema "unable to open database file" no Render.
"""
...
def criar_solucao_corrigida():
    """Cria os arquivos corrigidos para resolver o problema de persistência"""
    
    # 1. Criar um novo secure_api_service.py sem dependência do APIPersistence
    secure_api_service_code = '''#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
...
'''
    ...
    with open('backend/services/secure_api_service_corrigido.py', 'w', encoding='utf-8') as f:
        f.write(secure_api_service_code)
    
    # Também criar para o deploy
    with open('backend-deploy-render/services/secure_api_service_corrigido.py', 'w', encoding='utf-8') as f:
        f.write(secure_api_service_code)
    
    print("✅ Arquivo secure_api_service_corrigido.py criado")

def criar_app_corrigido():
    """Cria uma versão corrigida do app.py"""
    
    # Ler o app.py atual
    with open('backend/app.py', 'r', encoding='utf-8') as f:
        app_content = f.read()
    
    # Substituir a importação e inicialização do SecureAPIService
    ...
    # Remover as linhas de inicialização complexa do APIPersistence
    lines = app_content.split('\n')
    new_lines = []
    skip_lines = False
    
    for line in lines:
        if '# Garanta que a APIPersistence use o caminho correto do DB' in line:
            skip_lines = True
            # Adicionar a nova inicialização simples
            new_lines.append('    # Inicializar SecureAPIService (versão corrigida)')
            new_lines.append('    secure_api_service = SecureAPIService(app)')
            continue
    ...
    app_content_corrigido = '\n'.join(new_lines)
    
    # Escrever o arquivo corrigido
    with open('backend/app_corrigido.py', 'w', encoding='utf-8') as f:
        f.write(app_content_corrigido)
    
    # Também criar para o deploy
    with open('backend-deploy-render/app_corrigido.py', 'w', encoding='utf-8') as f:
        f.write(app_content_corrigido)
    
    print("✅ Arquivo app_corrigido.py criado")

def main():
    """Executa a correção completa"""
    print("🔧 INICIANDO CORREÇÃO DEFINITIVA DA PERSISTÊNCIA DA API")
    print("=" * 60)
    
    try:
        # Criar os arquivos corrigidos
        criar_solucao_corrigida()
        criar_app_corrigido()
        ...
        print("=" * 60)
        print("\n📋 PRÓXIMOS PASSOS:")
        print("1. Substitua backend/services/secure_api_service.py pelo secure_api_service_corrigido.py")
        print("2. Substitua backend/app.py pelo app_corrigido.py")
        print("3. Faça o mesmo no backend-deploy-render/")
        print("4. Faça o deploy para o Render")
        print("5. Execute o diagnostico_completo_api.py novamente")
