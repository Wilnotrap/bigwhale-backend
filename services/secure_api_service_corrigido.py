...
class SecureAPIService:
    ...
    def get_user_api_credentials(self, user_id: int) -> Dict[str, Optional[str]]:
        """
        Retorna as credenciais de API descriptografadas de um usuário.
        Retorna um dicionário com 'api_key', 'api_secret', 'passphrase' ou None se não encontradas/descriptografáveis.
        """
        try:
            user = User.query.get(user_id)
            if not user:
            ...
            if not all([api_key_enc, secret_enc, passphrase_enc]):
                return {
                    'api_key': None,
                    'api_secret': None,
                    'passphrase': None,
                    'error': 'Credenciais incompletas no banco de dados'
                }
    ...
    def register_routes(self):
        ...
        @self.app.route('/api/user/credentials', methods=['GET'])
        @self.require_login
        def get_user_credentials_route():
            """Retorna as credenciais da API do usuário logado."""
            try:
                user_id = session['user_id']
                credentials = self.get_user_api_credentials(user_id)
                
                if credentials['error']:
                    # Retornar 404 se não houver credenciais ou erro na descriptografia
                    # Isso pode ser ajustado para 200 com has_credentials: false se o frontend precisar diferenciar
                    return jsonify({
                        'success': False,
                        'message': credentials['error'],
                        'code': 'CREDENTIALS_NOT_FOUND_OR_DECRYPT_ERROR'
                    }), 404
                ...
                return jsonify({
                    'success': False,
                    'error': f'Erro ao buscar credenciais do usuário: {str(e)}',
                    'code': 'INTERNAL_ERROR'
                }), 500
